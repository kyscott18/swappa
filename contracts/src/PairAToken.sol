...
contract PairAToken is ISwappaPairV1 {
  ...
  function swap(
    ...
    if (inputType == 1) {
      ...
      require(ERC20(input).approve(lendingPoolCoreAddr, inputAmount), "PairAToken: approve failed!");
      ILendingPool(lendingPoolAddr).deposit(input, inputAmount, 0x0);
    }
    require(ERC20(output).transfer(to, inputAmount), "PairAToken: transfer failed!");
  }
