...
contract SwappaRouterV1 is ISwappaRouterV1 {
  ...
  function getOutputAmount(
    ...
    for (uint256 i; i < pairs.length; i++) {
      outputAmount = ISwappaPairV1(pairs[i]).getOutputAmount(path[i], path[i + 1], outputAmount, extras[i]);
    }
  }
  ...
  function swapExactInputForOutput(
    ...
    require(outputAmount >= minOutputAmount, "SwappaRouter: Insufficient output amount!");
    require(ERC20(output).transfer(to, outputAmount), "SwappaRouter: Final transfer failed!");
  ...
  function swapExactInputForOutputWithPrecheck(
...

    {
      ...
      for (uint256 i; i < pairs.length; i++) {
        ...
        bytes memory data = extras[i];
        outputAmount = ISwappaPairV1(pairs[i]).getOutputAmount(input, output, outputAmount, data);
      }
      require(outputAmount >= minOutputAmount, "SwappaRouter: Insufficient expected output amount!");
    }
