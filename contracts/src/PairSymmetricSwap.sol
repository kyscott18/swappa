...
contract PairSymmetricSwap is ISwappaPairV1 {
  ...
  function swap(
    ...
    require(ERC20(input).approve(swapPoolAddr, inputAmount), "PairSymmetricSwap: approve failed!");
    ISymmetricSwap(swapPoolAddr).swap(input, output, inputAmount);
    require(ERC20(output).transfer(to, inputAmount), "PairSymmetricSwap: transfer failed!");
  }
